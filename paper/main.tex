...
Nakajima and Sun \cite{nakajima2023sharp} (see also
\cite{altschuler2024capacity}), this gives the lower bound with
...
$V_s = \bigl(-\tfrac{a_2}{q_0}\sqrt{q_0}\,Z - \tfrac{a_1}{\psi_0}\bN\bigr)/D
 + s\bN$ with $Z$ standard Gaussian,
$\bN=\cE(-\gamma Z)/\sqrt{1-q_0}$ for $\gamma=\sqrt{q_0/(1-q_0)}$, and
$D=\sqrt{1-a_2^2/q_0}$, a one-dimensional Gaussian integral; and
...
$\{a^\star+tv:\, 0\le t\le T(v)\}$, which contains every cell the stage-2
sweep skips. The second derivative splits
...
a coarse box) and $1337$ leaf cells (stage~2, the box minus the star), all
...
European Union's Horizon 2022 research and innovation programme, the Simons
...
majorants and their pinned identities, the $B_\Lambda$ bound and the
sublevel-set localization, the corrected near-one chain), essentially all
of the code, the soundness audits, and the drafting of this manuscript.
The system operated in an autonomous goal mode: the author set the
objective of resolving the outstanding conditions, and it worked toward
that objective over extended unattended sessions, choosing the approach,
